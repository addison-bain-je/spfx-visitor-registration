...
interface IDataTableState {
    ListData: VisitorTypeItem[];
    showForm: boolean;
    _VisitorTypeItem: VisitorTypeItem;
    editForm: boolean;
}
...
class VisitorTypeDataTable extends React.Component<IProps, IDataTableState>{
        ...
        this.viewFields = [
            {
                ...
                render: (item: any) => {
                    return <Link style={{ color: 'orange' }}
                        onClick={() => {
                            this.openForm("VisitorType", item['ID'], false);
                        }}
                    >{item['VisitorType']}</Link>;
                }
            },
            {
                name: 'Description',
    ...
    private _getSelection = (items: VisitorTypeItem[]) => {
        this._selection = items;
    }

    private getData(): void {
        this._service.getAllrecords("VisitorType").then((result: VisitorTypeItem[]) => {
    ...
    private openForm(listname: string, id: number, edit: boolean): void {
        this._service.getItemById(listname, id).then((item) => { this.setState({ showForm: true, editForm: edit, _VisitorTypeItem: item }); });
    }
            ...
            <>
                <div>
                    <Grid container justify="flex-end">
                        <Typography className={classes.viewtitle} variant='subtitle2'>Visiting Type Profile</Typography>
                    </Grid>
                    
                    <Button className={classes.button} variant="text" startIcon={<AddIcon />} color="primary" onClick={() => { this.setState({ showForm: true, editForm: true, }); }}>New Visiting Type</Button>
                    <ListView
                    ...
                    >
                        <DialogTitle>Visitor Type Profile</DialogTitle>
                        <DialogContent>

                            <VisitorTypeForm
                                context={this.props.context}
                                formInitialValues={this.state._VisitorTypeItem}
                        ...
                        <DialogActions>
                            <Button color="primary" type="submit" form="VisitorTypeForm" disabled={!this.state.editForm}> save</Button>
