...
class _FVP_My_Actions extends React.Component<IProps, IDataTableState>{
    ...
    private _get_Data_by_Current_Handlers(): void {
        this._fvpService.getAllrecords("FVP").then((result: newItem) => {
            this.setState({ listID: result.listID });
            if (result.FvpItem.length > 0) {
                var FilteredItems_CurrentHandler: fvpItem[] = [];
                FilteredItems_CurrentHandler = result.FvpItem.filter((obj) => {
                    if (obj.CurrentHandler != '' && obj.CurrentHandler != null && obj.CurrentHandler != 'undefined') {
                        return obj.CurrentHandler.toString().indexOf((this.props.context.pageContext.user.email.toString())) >= 0;
                    }
                });
                if (FilteredItems_CurrentHandler.length > 0) {
                    let sortedInput = FilteredItems_CurrentHandler.slice().sort((a, b) => b.ID - a.ID);
                    //console.log("sortedInput 1 is " + JSON.stringify(sortedInput, null, 2));
                    this.setState({ DataByHandlers: sortedInput });
                }
            }
        });
    }
    ...
    public componentDidMount() {
        ...
        //this.getData();
        this._get_Data_by_Current_Handlers();
    ...
    private exportAllToExcel = () => {
        var result: any[] = [];
        (this.state.DataByHandlers).map(item => {
            var obj = {};
            ...
            <>
                <div>
                    {/* <Grid container justify="flex-end">
                        <Typography className={classes.viewtitle} variant='subtitle2'>My Actions</Typography>
                    </Grid> */}
                    ...
                    <ListView
                        items={this.state.DataByHandlers}
                        showFilter={true}
...
export default withStyles(styles)(_FVP_My_Actions);